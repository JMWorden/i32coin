...
func (bc *Blockchain) Listen(in <-chan int32coin.LocalMsg, out chan<- int32coin.LocalMsg) {
	for msg := range in {
		switch msg.Mtype {
		case int32coin.AddBlock:
			added := bc.AddBlock(msg.Block.(*Block), msg.Miner.(Hash))
			if added {
				out <- int32coin.LocalMsg{Mtype: int32coin.ShareBlock, Block: bc.Top()}
			}
			break
		...
		case int32coin.GenCandidate:
			b := bc.CandidateBlock()
			out <- int32coin.LocalMsg{Mtype: int32coin.CandidateBlock, Block: b}
			break
		case int32coin.ReqHeight:
			out <- int32coin.LocalMsg{Mtype: int32coin.Height, Height: bc.height}
		}
	}
}
